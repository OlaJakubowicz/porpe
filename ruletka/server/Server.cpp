...
struct UnitOfClientWork{
  ...
  void operator()()
  {
    ...
    int i,count = atoi( file_count.c_str() );
	double rnd,sum=0;
	double prop[queue->size()+1];
	
	for (i = 0; i < queue->size(); i++){
       sum = sum + queue->ServerWorkerAt(i);
    	}     
       
    prop[0]=10.0*(1.0-(queue->ServerWorkerAt(0)/sum));
    for (i = 1; i < queue->size(); i++){
       prop[i] =prop[i-1] + 10.0*( 1.0 -(queue->ServerWorkerAt(i)/sum));
    }
    sum=prop[queue->size()-1];
    
    prop[0]=prop[0]/sum;
    for (i = 1; i < queue->size(); i++){
       prop[i] = prop[i-1]+ prop[i]/sum;
    }

	int size=100*(int)prop[queue->size()-1] ;
   	rnd = rand()%size;   	
//	cout << "dlaf fi " << prop[queue.size()-1] << "  "<< endl;
   	
    for (i = 1; i < queue->size()+1; i++) {
        if ((rnd/size > prop[i-1]) && (rnd/size < prop[i])) break;    
    }
    
    if (i >= queue->size()) i = queue->size();
	
	ServerWorker* server_worker = queue->popServerWorkerAt(i);
